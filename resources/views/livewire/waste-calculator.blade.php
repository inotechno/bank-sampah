    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-12 px-4 sm:px-6 lg:flex-row lg:px-8">
        <section class="flex-1 rounded-3xl border border-emerald-100 bg-white/80 p-6 shadow-xl backdrop-blur">
            <header class="space-y-3 border-b border-emerald-50 pb-6">
                <span class="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium uppercase text-emerald-700">
                ...
                <p class="text-sm text-emerald-800/80">
                    ...
                    Nilai ini bersifat perkiraan berdasarkan harga per kilogram yang telah ditetapkan oleh pengelola bank sampah.
                </p>
            </header>

            <div class="mt-8 space-y-6">
                @forelse ($entries as $index => $entry)
                    <article wire:key="entry-{{ $index }}" class="rounded-2xl border border-emerald-100 bg-white/90 p-5 shadow-sm ring-1 ring-transparent transition hover:shadow-md hover:ring-emerald-200">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div class="w-full sm:w-1/2">
                                <label for="waste-type-{{ $index }}" class="text-sm font-medium text-emerald-900">Jenis Sampah</label>
                                <div class="mt-1">
                                    <select
                                        id="waste-type-{{ $index }}"
                            ...
                            <div class="w-full sm:w-1/3">
                                <label for="weight-{{ $index }}" class="text-sm font-medium text-emerald-900">Berat Sampah (kg)</label>
                                <div class="mt-1 relative">
                                    <input
                                        ...
                                        placeholder="0,0"
                                    />
                                    <span class="pointer-events-none absolute inset-y-0 right-4 flex items-center text-xs font-semibold uppercase tracking-wide text-emerald-500">kg</span>
                                </div>
                        ...
                        @php
                            $selectedType = $wasteTypes->firstWhere('id', $entry['waste_type_id'] ?? null);
                        @endphp
                        @if ($selectedType)
                            <dl class="mt-4 grid grid-cols-1 gap-3 text-sm text-emerald-800 sm:grid-cols-2">
                                <div class="rounded-xl bg-emerald-50/70 px-3 py-2">
                                    ...
                                    <dd class="mt-1 text-base font-semibold text-emerald-900">Rp{{ number_format($selectedType->price_per_kg, 0, ',', '.') }}</dd>
                                </div>
                                <div class="rounded-xl bg-emerald-50/70 px-3 py-2">
                                ...
                                @if ($selectedType->description)
                                    <div class="sm:col-span-2 rounded-xl border border-emerald-100 bg-white px-3 py-2 text-xs text-emerald-700">
                ...
                @empty
                    <p class="rounded-2xl border border-dashed border-emerald-200 bg-white/80 p-6 text-center text-sm text-emerald-700">Belum ada jenis sampah yang dimasukkan.</p>
                @endforelse
            </div>
            ...
            <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
                        <path d="M12 2a1 1 0 0 1 .911.59l8 18a1 1 0 0 1-1.822.82L16.382 16H7.618l-2.707 5.41a1 1 0 0 1-1.822-.82l8-18A1 1 0 0 1 12 2Zm3.382 12-3.382-6.764L8.618 14h6.764Z" />
                    </svg>
                    Hitung Total
                </button>
            </div>
            ...
            <footer class="mt-8 rounded-2xl border border-emerald-100 bg-emerald-50/80 p-6">
                <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-3xl font-bold text-emerald-900">Rp{{ number_format($summary['total'], 0, ',', '.') }}</p>
                    </div>
                    <div class="text-sm text-emerald-700/80">
                        Estimasi nilai setoran ini akan dikonfirmasi kembali oleh petugas setelah proses penimbangan di lokasi bank sampah.
                    </div>
                </div>
            </footer>
        </section>
        ...
        <aside class="w-full max-w-xl space-y-6">
            <div class="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-lg">
                ...
                <p class="mt-2 text-sm text-emerald-700/80">Pantau perincian setiap jenis sampah untuk memastikan hitungan sesuai dengan berat yang Anda masukkan.</p>
                ...
                <ul class="mt-6 space-y-4">
                        ...
                        <li class="rounded-2xl border border-emerald-100 bg-emerald-50/80 px-4 py-3">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-xs text-emerald-600">{{ number_format($row['weight'], 1, ',', '.') }} kg × Rp{{ number_format($row['price'], 0, ',', '.') }}</p>
                                </div>
                                <p class="text-sm font-bold text-emerald-900">Rp{{ number_format($row['total'], 0, ',', '.') }}</p>
                            </div>
                    ...
                    @empty
                        <li class="rounded-2xl border border-dashed border-emerald-200 bg-white/70 px-4 py-6 text-center text-sm text-emerald-700/70">
                            Data ringkasan akan muncul setelah Anda memilih jenis sampah dan memasukkan beratnya.
                        </li>
                    @endforelse
                </ul>

                <div class="mt-6 flex items-center justify-between rounded-2xl border border-emerald-200 bg-white px-4 py-3">
                    <p class="text-sm font-semibold text-emerald-900">Perkiraan Total</p>
                    <p class="text-xl font-bold text-emerald-600">Rp{{ number_format($summary['total'], 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-lg">
                ...
                <p class="mt-2 text-sm text-emerald-700/80">Jenis-jenis berikut memiliki harga per kilogram tertinggi dan belum Anda tambahkan.</p>

                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    @forelse ($recommendations as $type)
                        <button
                            type="button"
                            wire:click="quickAdd({{ $type->id }})"
                        ...
                        >
                            <div>
                                <p class="text-sm font-semibold text-emerald-900 group-hover:text-emerald-700">{{ $type->name }}</p>
                                <p class="text-xs text-emerald-600">Rp{{ number_format($type->price_per_kg, 0, ',', '.') }}/kg</p>
                            </div>
                            <span class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 group-hover:text-emerald-700">
                    ...
                    @empty
                        <p class="rounded-2xl border border-dashed border-emerald-200 bg-white/70 px-4 py-6 text-center text-sm text-emerald-700/70">
            ...
            <div class="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-500 via-emerald-600 to-emerald-700 p-6 text-white shadow-xl">
                <h2 class="text-lg font-semibold">Tips Maksimalkan Setoran</h2>
                <ul class="mt-4 space-y-3 text-sm">
                    <li class="flex items-start gap-3">
                        <span class="mt-1 inline-flex size-6 items-center justify-center rounded-full border border-white/30 bg-white/10 text-xs font-bold">1</span>
                        <span>Pisahkan sampah berdasarkan jenisnya untuk mempercepat proses penimbangan.</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <span class="mt-1 inline-flex size-6 items-center justify-center rounded-full border border-white/30 bg-white/10 text-xs font-bold">2</span>
                        <span>Pastikan sampah dalam keadaan bersih dan kering agar harga tetap optimal.</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <span class="mt-1 inline-flex size-6 items-center justify-center rounded-full border border-white/30 bg-white/10 text-xs font-bold">3</span>
                        <span>Gunakan fitur rekomendasi untuk melihat jenis sampah dengan nilai jual tertinggi saat ini.</span>
                    </li>
                </ul>
            </div>
        </aside>
